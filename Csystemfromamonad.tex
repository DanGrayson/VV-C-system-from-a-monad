...
Next, for a subset $X$ of $V$ let $Exp^{\cdot}_{Sig_2}(X,Y)$\DG{How does $Exp^{\cdot}_{Sig_2}(X,Y)$ differ from $Exp_{Sig_2}[X,Y]$?}
\BA{Maybe that $t \in Exp^{\cdot}_{Sig_2}(X,Y)$ only if all $x \in X$ \textbf{do} occur freely in $t$?}
be the subset of
...
necessary and sufficient for a type theory to ``make sense''.\DG{
  Mention where we see that in this paper, and state which properties are the structural properties.}
\BA{Vladimir describes the B-system \textbf{operations} on the B-system obtained from a C-system, but not the properties that these operations satisfy. Maybe he ran out of time/energy?}
...
\BA{This paragraph introducing notation would be more suitable at the beginning of the section on C-systems, Section~\ref{C-systems}. (Check for duplication!)}
...
\BA{I think the part starting here could be under the section title ``Future work''. Vladimir gives a roadmap with quite precise specifications of constructions that build upon this paper.}

The sets $(Ob,\wt{Ob})$ are equipped with a system of operations
$(l,ft,\partial,T,S,\wt{T},\wt{S},\delta)$. Pairs of sets equipped with
operations of such form are called (unital) pre-B-systems (cf.{} \cite{Bsystemsnew}\DG{What reference to B-systems is intended here?}).\index{pre-B-systems}
...
conditions are shown in Proposition X.A\DG{Apparently this proposition was not added to the paper.}\BA{Indeed, it is missing.} of the present paper to be
...
we define, in Construction X.B\DG{Apparently this construction was not added to the paper.}\BA{Yes.}, a pre-B-system structure on the sets
$(B(\RR,\LM),\wt{B}(\RR,\LM))$ and, in Construction X.C\DG{Apparently this construction was not added to the paper.}\BA{Yes.}, an isomorphism
...
Summing it up in Construction X.D\DG{Apparently this construction was not added to the paper.}\BA{Yes.} we obtain a construction, for any four
...
$(B,\wt{B})$\DG{Strange phrasing, since $(B,\wt{B})$ is a pair of sets, not a set.} that satisfy the conditions of \cite[Prop. 5.4]{Csubsystems}. The
...
C-systems $CC$ (see below).\editorfootnote{The description of the pre-B-systems $(B(CC[F]),\wt{B}(CC[F]))$
  seems not have been achieved.}\DG{How can we publish the paper without the main result?
  On the other hand, this text is from the part of the introduction we were supposed to ignore.}
...
except possibly the s-morphism condition\DG{Is it clear what this condition is?}, is a homomorphism.
%
\begin{lemma}
\label{2015.08.22.l4}
