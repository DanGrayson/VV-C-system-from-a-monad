...
\newglossaryentry{monad in monoidal form}{name={monad in monoidal form},description={}}
...
\glsxtrnewsymbol[description={a functor from relative monads to C-systems}]{C}{\ensuremath{C}}
...
\begin{definition}
  ...
  A {\em \gls{monad in monoidal form}}\index{monad!in monoidal form}\editorfootnote{formalized as the type {\tt Monad} in \cite{UniMath}.}
...
A {\em monad in Kleisli form}\index\index{monad!in Kleisli form}\editorfootnote{formalized as the type {\tt KleisliMonad} in \cite{UniMath}.} on a category $\C$ is a triple $$\RR=(RR_{Ob},\eta,rr),$$ where 
%
\begin{enumerate}
...
monad. The problem is to construct a functor $\RR^{lm}:K(\RR)\sr \D$.
\end{problem}
%
...
and given a $J$-monad $\RR$, to construct a functor $F_{\RR}:K(F^{\circ}(\RR))\sr
K(\RR)$.
\end{problem}
%
\begin{construction}\rm
...
\begin{definition}
  ...
  An {\em object constancy structure} on a functor $F:\C\sr\D$
...
Let $\C$ and $\D$ be categories. Let $\RR$ be an endo-monad on $\C\times\D$. Let $A\in Ob(D)$. The problem is to construct an endo-monad $\RR_{1,A}$ on $\C$.
\end{problem}
%
\begin{construction}
