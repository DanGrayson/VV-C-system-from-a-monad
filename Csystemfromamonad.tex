...
function has a well defined domain and codomain\editorfootnote{Thus a function is a triple
  consisting of the domain, the codomain, and the graph of the function, as a set of pairs.}.
Then the union in (\ref{2017.02.24.eq1}) is disjoint and we set
%
...
expression'' and ``expression that describes an element''.\editorfootnote{
  ...
  written with ZFC as metatheory, but in a paper on a general mathematical
  topic with Univalent Foundations as the metatheory, the convention would not
...
In the theories we consider, if the sentence (\ref{2017.02.06.eq4}) is valid than so are the sentences
...
conditions are shown in Proposition \ref{} of the present paper\editorfootnote{I can't find such a proposition.} to be
...
\editorfootnote{Where's the definition of $B(\RR,\LM)$, etc.?  Does it amount to a construction of a B-system directly from a relative monad?}
$(B(\RR,\LM),\wt{B}(\RR,\LM))$ and, in Construction \ref{}, an isomorphism%
\editorfootnote{Did that part of the paper not get written?}
...
\editorfootnote{Did that construction not get written?  Probably not, as the symbol $Beq$ occurs only in the introduction.}
...
The following construction, together with Construction \ref{}, is used to
...
The problem is to construct a structure of a $J_{\D}$-monad on $RR_{\D}$. This $J_{\D}$-monad will be denoted $\RR_{\D}$.
\end{problem}
%
\begin{construction}\rm
...
pr_{\C}=Id_{\C}$ it is an endo-monad on $\C$, which we denote by
...
\llabel{onCsystems}
%
The definition of ``C-system'' is given in \cite[Def. 2.1,
