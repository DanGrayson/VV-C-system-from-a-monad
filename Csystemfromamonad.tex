...
construction of the term C-systems of type theories.

AMS Subject Classification: 18C15, 03B60.
\end{abstract}
...
existing body of work on type theory, as reflected in \cite[Theorem
  1.2.9]{KLV1}.\editorfootnote{Statement 1.2.9 of [26] was presented as a theorem in
  early versions of the article, but starting with the 5th arXiv version (posted after
  the death of Voevodsky) it was presented as a conjecture, reflecting
  Voevodsky’s and the authors’ concerns over its status in the literature.   It
  states that the C-system (contextual category) associated to
  the particular type theory under consideration is an initial object in the category of C-systems
  with the appropriate extra structure.}
(See also \cite[Example 1.2.3]{KLV1}\editorfootnote{Example 1.2.3 of \cite{KLV1}
  asserts that for the particular type theory under consideration, one may explicitly construct
  the corresponding C-system.  The present paper and \cite{Csubsystems} can be regarded
  as filling in the details.},
which claims as obvious
...
described by Fiore, Plotkin and Turi in \cite{FPT}\footnote{Warning: the paper uses the name ``presheaves'' for {\em covariant} functors to the category of sets.}. Later a different abstraction,
...
uses the axiom of choice.\editorfootnote{Tarmo Uustalu, co-author of \cite{ACU} and \cite{ACU2},
  has confirmed that the proof does use the axiom of choice, in the proof of Theorem 4.2 (3) of
  \cite{ACU2}, which basically asserts that the set of functions from a set $X$
  to a quotient of a set $Y$ is a quotient of the set of functions from $X$ to $Y$.}
...
second-order signatures, see \cite{DBLP:conf/cpp/AhrensMM22}.
