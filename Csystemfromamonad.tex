...
%### Is it true? Ask Fiore to check and find references
...
\end{enumerate}
%
We leave the verification of the conditions and the proof that these functions are mutually inverse to the formally verified version of the paper.
\ec 
%#### details, to M.
For a monad $\RR$ in the Kleisli form we let $\RR^M$ denote the corresponding monad in the monoidal form and a monad $\R$ in the monoidal form we let $\R^K$ denote the corresponding monad in Kleisli form.

The notion of a relative monad arises very naturally for monads in the Kleisli form. It was introduced in \cite[Def.1, p.299]{ACU} and considered in more detail in \cite{ACU2}. Let us remind it here. 
%
\begin{definition}
...
\item for all $X$ in $\C$, a morphism $\eta_{X}:J(X)\sr RR_{Ob}(X)$ in $\D$,
\item for all $X,Y$ in $\C$ and $f:J(X)\sr RR_{Ob}(Y)$ in $\D$, a morphism in $\D$
%
...
Given a $J$-relative monad $\RR=(RR_{Ob},\eta,rr)$ to construct a function $RR_{Mor}:Mor(\C)\sr Mor(\D)$ such that $\RR^f=(RR_{Ob},RR_{Mor})$ is a functor. 
\end{problem}
%
\begin{construction}\rm\llabel{2016.01.15.constr1}\rm
For $f:X\sr Y$ in $\C$ set
%
\begin{eq}\llabel{2017.04.05.eq3}
RR_{Mor}(f)=rr_{X,Y}(J(f)\circ \eta_{Y})
\end{eq}
%
The proofs of the composition and the identity axioms of a functor are straightforward. 
\ec
%### details, to M. or J.?
%
An $Id_{\C}$-relative monad will be called an endo-monad. An endo-monad is precisely a monad on the corresponding category given in the Kleisli form. This permits us to call, sometimes, the endo-monads simply monads. As the explicit form of functions $(-)^K$ and $(-)^M$ defined in Construction \ref{2017.01.04.constr1} shows that for $\R=(R,\eta,\mu)$ we have $\R^K=(R_{Ob},\eta,rr)$ and for $\RR=(RR_{Ob},\eta,rr)$ we have $\RR^M=(\RR^f,\eta,\mu)$. In particular, since $(\R^K)^M=\R$, we have
%
\begin{eq}\llabel{2017.04.05.eq2}
(\R^K)^f=R
\end{eq}
%
...
%
\begin{eq}\llabel{2017.04.05.eq1}
...
We omit the index $\RR$ at $Et$. We set $Et_{Ob}=Id$. For a morphism $f:X\sr Y$ in $\C$ we set 
%
\begin{eq}
...
$$$$J(f)\circ \eta_Y\circ rr_{Y,Z}(J(g)\circ \eta_Z)=J(f)\circ J(g)\circ \eta_Z$$
%
where the second equality is by (\ref{2017.04.05.eq1}) and the third by condition (2) of Definition \ref{2015.12.22.def1}. This proves the composition axiom. 
\ec
%
\begin{problem}\llabel{2017.04.11.prob1}
Let $J:\C\sr\D$ be a functor and $\RR=(RR_{Ob},\eta,rr)$ a $J$-relative monad. To construct a functor $\RR^{lm}:K(\RR)\sr \D$.
\end{problem}
%
...
%
where the equality is actually the bijection mentioned in the definition of $K(\RR)$, we set $\RR^{lm}_{Mor}(f)=rr_{X,Y}(f)$. 

The verification of the composition and identity axioms for $(\RR^{lm}_{Ob},\RR^{lm}_{Mor})$ is straightforward and is left for the formalized version of the paper.
\ec
%
%### details to M.?
...
Given functors $F:\C_0\sr \C_1$, $J:\C_1\sr\D$ and a $J$-relative monad $\RR$ to construct a $(F\circ J)$-relative monad $F^{\circ}(\RR)$.
\end{problem}
%
\begin{construction}\llabel{2017.02.24.constr1}\rm
We omit the indexes $Ob$ and $Mor$ at $F$ and $J$. Let $\RR=(RR_{Ob},\eta,rr)$. We set
%
\begin{eq}
...
F^{\circ}(RR_{Ob})(X)=RR_{Ob}(F(X))\spc\spc F^{\circ}(\eta)_X=\eta_{F(X)}
\end{eq}
%
...
\end{eq}
%
Let us verify conditions (1)-(3) of Definition \ref{2015.12.22.def1}. We write $RR$ instead of $RR_{Ob}$ :
%
\begin{enumerate}
...
\end{eq}
%
where the first equality is by Construction \ref{2017.03.12.constr3} and the second by Construction \ref{2017.02.24.constr1}. The data $(F_{\RR,Ob},F_{\RR,Mor})$ satisfies the identity axiom because of the equalities
%
...
\end{eq}
%
where, again, the first equality is by Construction \ref{2017.03.12.constr3} and the second by Construction \ref{2017.02.24.constr1}. The data $(F_{\RR,Ob},F_{\RR,Mor})$ satisfies the composition axiom because of the equalities
%
$$F_{\RR,X,Z}(f\circ_{K(F^{\circ}(\RR))} g)=F_{\RR,X,Z}(f\circ_{\D} rr_{F(Y),F(Z)}(g))=
$$$$f\circ_{\D} rr_{F(Y),F(Z)}(g)=f\circ_{K(\RR)}g=F_{\RR,X,Y}(f)\circ_{K(RR)}F_{\RR,Y,Z}(g)$$
%
where the first equality is by (\ref{2017.03.12.eq3}), the second by (\ref{2017.03.12.eq2}), the third by Construction \ref{2017.03.12.constr3} and the fourth again by (\ref{2017.03.12.eq2}). 

This completes Construction \ref{2017.03.12.constr2}.
\ec
%

The following construction together with Construction \ref{} are used to extract pairs $(\RR,\LM)$ of a relative monad and a left module over it from two-sorted relative monads such as the ones that arise from two-sorted second-order signatures, see \cite{AMM2017}. 
...
%
where the first equality holds by (\ref{2017.04.21.eq2}), the second since $pr_{\D}$ commutes with compositions, the third by (\ref{2017.04.21.eq5}), and the fourth again by (\ref{2017.04.21.eq2}). This proves the property \ref{2017.04.13.def1}\ref{2017.04.19.eq6} for $\RR_{\D}$ and completes Construction \ref{2017.04.21.constr1}.
\ec
%
We want to emphasize the following particular case of Construction \ref{2017.04.21.constr1}.
%
\begin{problem}
...
Let $J:\C\sr\C\times\D$ be the functor given by $X\mapsto (X,A)$ on objects and $f\mapsto (f,Id_A)$ on morphisms. Consider the $J$-monad $J^{\circ}(\RR)$. It satisfies the conditions of Problem \ref{2017.04.21.prob1} with respect to the identity object constancy structure corresponding to $A$. Therefore, Constructions \ref{2017.04.21.constr1} applies and we obtain a $J\circ pr_{\C}$-monad $(J^{\circ}(\RR))^{\D}$. Since $J\circ pr_{\C}=Id_{\C}$ it is an endo-monad on $\C$ that we denote by $\RR_{1,A}$. Explicitly, for $\RR=(RR,\eta,rr)$, we have $\RR_{1,A}=(RR_{1,A},\eta^{1,A},rr^{1,A})$ where:
%
\begin{enumerate}
...
%
\begin{remark}\rm
The notation $\RR_{1,A}$ emphasizes that the monad is on the first projection of the product $\C\times\D$. We can also construct, for $A\in Ob(C)$, an endo-monad $\RR_{2,A}$ on $\D$. 
\end{remark}
%
...
\end{enumerate}
%
We will say that $\LM$ is an $\RR$-module if it is a left $\RR$-module. We will say that $\LM$ is a module over $\RR$ without specifying $\E$ if $\E=\D$. 
\end{definition}
%
From the unfolded definition we see that the left modules over an $Id$-relative monad are exactly the same as the left modules in the Kleisli form over the corresponding monad. Following the notation $\R^K$ for the monad in the Kleisli form corresponding to a monad $\R$ in the monoidal form, we let ${\bf L}^K$ denote the module over $\R^K$ in the Kleisli form corresponding to a module ${\bf L}$ over $\R$ in the monoidal form.
%
\begin{definition}
...
Given functors $F:\C_0\sr \C_1$ and $J:\C_1\sr \D$, a $J$-monad $\RR$ and an $\RR$-module $\LM$ with values in $\E$ to construct an $F^{\circ}(\RR)$-module $F^{\circ}(\LM)$ with values in $\E$.
\end{problem}
%
\begin{construction}\rm
\llabel{2017.03.12.constr1}
We need to construct a functor $K(F^{\circ}(\RR))\sr\E$. We define this functor as the  composition $F_{\RR}\circ \LM$, where $F_{\RR}$ is defined in Construction \ref{2017.03.12.constr2}. Explicitly, for $\RR=(RR_{Ob},\eta,rr)$ and $\LM=(LM_{Ob},lm)$, we let $F\crc(\LM)=(F\crc(LM_{Ob}),F\crc(lm))$. In this notation we have 
%
\begin{eq}
...
(F\circ \LM^f_{Mor})(f)$$
%
where the first equality is by (\ref{2017.04.11.eq5}), the second by definition of $F\circ J$ and (\ref{2017.04.11.eq1}), the third by (\ref{2017.04.17.eq8}), the fourth by (\ref{2017.04.11.eq5}) and the fifth by the definition of $F\circ \LM^f_{Mor}$. This completes the proof of Lemma \ref{2017.04.17.l1}.
\end{proof}
%
...
Let $\C_0,\C_1,\E$ be categories. Let $\R=(R,\eta,\mu)$ be a monad on $\C_1$ and $ {\bf  L}=(L,\rho)$ a left $\R$-module with values in $\E$. Let further $J:\C_0\sr \C_1$ be a functor. To construct a pair of a $J$-relative monad module with values in $\E$ over it.
\end{problem}
%
...
Constructions \ref{2017.02.24.constr1} and \ref{2017.03.12.prob2} show that both relative monads and left modules over them can be ``precomposed'' with any functor. The left modules can also be ``post-composed'' with any functor. It is done by literal post-composition. Since a left module is a functor $\LM:K(\RR)\sr \E$ we can post-compose it with any functor $F:\E\sr \E'$ and obtain a new module that we will denote $\LM\circ F$. Explicitly, for $\LM=(LM_{Ob},lm)$ one has
%
...
We need to construct a family $lm^{\D}$, parametrized by $X,Y\in\C$ and $f:J_{\D}(X)\sr RR_{\D}(Y)$, of morphisms $lm^{\D}_{X,Y}(f):RR(X)\sr RR(Y)$,
and to prove that $\LM_{\D}$, defined as $(RR,lm^{\D})$,  satisfies 
conditions (1) and (2) of Definition \ref{2017.04.15.def1}. 

We set:
%
$$lm^{\D}_{X,Y}(f)=rr_{X,Y}((f,\nu_{X,Y}\circ \eta^{\D'}_Y))$$
%
The proof of condition (1) of Definition \ref{2017.04.15.def1} is given by (\ref{2017.04.23.eq1}). The proof of condition (2) is given by (\ref{2017.04.21.eq5}).
This completes Construction \ref{2017.04.23.constr2}.
\ec
%
We also have an analog of the special case described in Construction \ref{2017.04.23.constr1}. Let $\C$,$\D$ be categories and $A\in Ob(\D)$. Let $J_{1,A}:\C\sr\C\times\D$ be the functor given by $X\mapsto (X,A)$ on objects and $f\mapsto (f,Id_A)$ on morphisms. In Construction \ref{2017.04.23.constr1} we wrote $J$ instead of $J_{1,A}$. 
%
\begin{problem}
\llabel{2017.04.23.prob3}
Let $\C$,$\D$ be categories. Let $\RR=(RR,\eta,rr)$ be an endo-monad on $\C\times\D$. Let $A\in Ob(D)$. To construct an $\RR_{1,A}$-module structure on $J_{1,A}\circ RR$.
\end{problem}
%
\begin{construction}
\llabel{2017.04.23.constr3}\rm
Consider the $J_{1,A}$-monad $J_{1,A}^{\circ}(\RR)$. It satisfies the conditions of Problem \ref{2017.04.23.prob2} with respect to the identity object constancy structure corresponding to $A$. Therefore, Constructions \ref{2017.04.23.constr2} applies and we obtain a $(J_{1,A}^{\circ}(\RR))_{\C}$-module structure on $J_{1,A}\circ RR$. Since, by Construction \ref{2017.04.23.constr1}, we have $\RR_{1,A}=(J_{1,A}^{\circ}(\RR))_{\C}$, this provides a construction for Problem \ref{2017.04.23.prob3}. Explicitly, if we let $(J_{1,A}\circ RR, lm^{1,A})$ denote this module, we have, for $X,Y\in \C$ and $f:X\sr Y$
%
...
If $F(pt)=\emptyset$ then $CC[F]=\{pt\}$. On the other hand, the choice of an element $y$ in $F(pt)$ defines distinguished elements $y_X=F(\pi_X)(y)$ in all sets $F(X)$ and therefore distinguished objects $(X,\Gamma_{X,y})=(X,(y,\dots,y_{ft^2(X)},y_{ft(X)}))$ in the fibers of the object component of $tr_{F}$ over all $X$. 
...
We will only consider the case when $U$ is a universe. As was mentioned in the introduction we fix $U$ and omit it from our notations below. 

Following \cite{ACU} we let $Jf:F\sr Sets$ denote the functor that takes $n$ to $stn(n)$ and that is the identity on morphisms between two objects (on the total sets of morphisms the morphism component of this functor is the inclusion of a subset). 
...
Given a monad ${\bf R}=(R,\eta,\mu)$ (cf. \cite{MacLane}[p.133]) on the category of sets to construct a $Jf$-relative monad $\RR$.
\end{problem}
%
...
The set of $Jf$-relative monads is in an easy to construct bijection with the set of abstract clones as defined in \cite[Section 3]{FPT}. 
\end{remark}
...
\end{proof}
%
Another operation that we would like to have an explicit form of is operation $\delta$. For a C-system $CC$ and an object $\Gamma$ in $CC$ such that $l(\Gamma)>0$ one defines $\delta_{\Gamma}$ as $s_{Id(\Gamma)}$ (cf. \cite[Section 3]{Csubsystems}). 
%
...
This completes the description of the pre-B-system structure on $(Ob,\wt{Ob}')$ that is obtained by the transport of structure from the standard pre-B-system structure on $(Ob,\wt{Ob})$ by means of the pair of isomorphisms $Id$ and $mb_{\RR,\LM}$.
%
\begin{remark}\rm
...
These l-versions of the relative monads and their modules should be easier to formalize in systems such as HOL.  
\end{remark} 
%
...
\item This material is based on research sponsored by The United States Air Force Research Laboratory under agreement number FA9550-15-1-0053. The US Government is authorized to reproduce and distribute reprints for Governmental purposes notwithstanding  any copyright notation thereon.
...
%### concept of a type theory in the Martin-Lof presentation - $\RR$+$\LM$+subsets $B_T$, $\wt{B}_T$, $Beq_{T}$, $\wt{Beq}_T$ satisfying the conditions of Theorem \ref{}. 

%### The concept of adding to a type theory of a sequence of constants can be described as a construction on the level of C-systems. Can we show that the concept of adding a sequence of definitional equalities can not?



In multi sorted predicate logic the quantifiers have two arguments - the sort of the variable being quantified and the expression. Correspondingly, their binding arity is $(0,1)$. The first argument is the sort, where no variables are bound, and second is the expression where one variable is bound. This example is also an example of a two-sorted binding signature since the first argument is of sort ``sort'' and the second is of sort ``formula''. The two sorted binding signatures are very important for this paper and we will come back to them later. 
%###
...
In the assumption of the ZFC with a Grothendieck universe $U$ as the only meta-theory various authors constructed an equivalence of categories between the category of finitary monads on the category of sets $Sets(U)$ and the less well known category of the $Jf$-relative monads with values in $Sets(U)$. There are further equivalences between being finitary relative to the concepts of a directed colimit or filtered colimit as well as convenient criteria of when a diagram is filtered. 

With UniMath as the second intended meta-theory, many of these equivalence proofs need to be rewritten and it is not at all obvious which ones can be preserved. What is clear, is that the concept of a $Jf$-relative monad remains the most general one. The categories of finitary monads, where being finitary is defined relative to either directed colimits or filtered colimits, as well as a number of other, classically equivalent, categories can all be equipped with obvious functors to the category of the $Jf$-relative monads. What is unclear is which of these functors can be proved, in UniMath, to be equivalences.  

In addition, a pair of a $Jf$-relative monad $\RR$ and a left module $\LM$ over it contains {\em exactly} that information which is used in the construction of the C-system $C(\RR,\LM)$. One could describe this construction as starting with a pair of a general monad on $Sets(U)$ and a left module over this monad. However, we do not know at this time whether one can construct, in UniMath, a monad from a $Jf$-relative monad nor what is the correct definition of being finitary that would allow us to prove, in UniMath,  that the functor from finitary monads to the $Jf$-relative monads is fully-faithful. 
...
Fortunately, we have a very good mathematical theory of abstract {\em algebraic} expressions %that goes back to Lawvere's thesis \cite{Lawvere}.
 In its most convenient for us form, one considers the sets $RR(X)$ of algebraic expressions with variables from the set $X$. One constructs, using the possibility to substitute an expression for a variable, first the functor structure and then the monad structure on this family of sets. It turns out that this monad contains, or can be equipped with, all of the structures that are important in the study of algebraic expressions. 

Not any monad arises in this way. The first, most fundamental, restriction on these monads is that they are finitary, that is, that the underlying functor commutes with directed colimits. This property corresponds to the fact that even if we have an infinite set from which variables can be taken any particular expression contains only finitely many variables. 

The second restriction is that, for algebraic expressions, these monads are {\em free} monads in the sense that is made precise for example in  \cite[Prop. 10.14]{Awodey2006}. 

Expressions that one has to deal with in type theory {\em are not} algebraic expressions. The simplest example is the system of expressions underlying the type theory that is called the simply typed $\lambda$-calculus with the set of generating types $G$. The expressions of this type theory are the ones that are necessary to accommodate the following inference rules:

$$
\frac{}{\rh}\spc\spc\spc\spc\spc
\frac{x_0:T_0,\dots,x_{n-1}:T_{n-1}\rh}{x_0:T_0,\dots,x_{n-1}:T_{n-1}\rh x_i:T_i}\,\,\,(0\le i<n)
$$$$\frac{\Gamma\rh o:T\spc\Gamma\rh o':T\spc o\sim^{\alpha}o'}{\Gamma\rh o\equiv o':T}
$$$$\frac{\Gamma\rh}{\Gamma\rh T\type}\,\,\,(T\in G)\spc
\frac{\Gamma,x:A\rh B\type}{\Gamma\rh \prod(A,B)\type}
$$$$\frac{\Gamma,x:A\rh o:B}{\Gamma\rh \lambda(A,x.o):\prod(A,B)}\spc
\frac{\Gamma\rh o:A\spc \Gamma\rh f:\prod(A,B)}{\Gamma\rh ap(f,o):B}
$$$$\frac{\Gamma,x:A\rh o_1:B\spc\Gamma\rh o_2:A}{\Gamma\rh ap(\lambda(A,x.o_1),o_2)\equiv o_1[o_2/x]:B}\spc
\frac{\Gamma\rh f:\prod(A,B)}{\Gamma\rh \lambda(A,x.ap(f,x))\equiv f:\prod(A,B)}$$
%

A short explanation is required here. For a symbol $S$, expressions $E_1,\dots,E_d$ and names of variables $x_{1}^1,\dots,x_{n_1}^1;\dots;x_{1}^d,\dots,x_{n_d}^d$, the sequence
%
$$S(x_{1}^1.\dots.x_{n_1}^1.E_1,\dots,x_{1}^d.\dots.x_{n_d}^d.E_d)$$
%
denotes an expression where variables $x_{1}^i,\dots,x_{n_i}^i$ are bound in the sub-expression $E_i$. 



The expressions that are necessary to accommodate these inference rules are as follows: 
%
\begin{enumerate}
\item the name of a free variable is an expression - this is required by the second rule,
\item for $T\in G$, $T$ is an expression - this is required by the fourth rule,
\item if $A$ and $B$ are expressions, then $\prod(A,B)$ is an expression - this is a little stronger than what is required by the fifth rule, but we will keep it like that for now,
\item if $A,o$ and $B$ are expressions and $x$ is not free in $A$ then $\lambda(A,x.o)$ is an expression and $x$ is bound in the $o$ part of this expression - this is required by the 
...
Some things need to be explained here. The first three rules are present in the inference rules list of any type theory. The first one asserts that the empty context is a valid one. The second that a variable declared in a valid context is a valid element expression of the corresponding type in this context. 

The third one is of great importance for what will follow. The notation $o\sim o'$ 
...
This monad on the category of sets, or its relative version -- a $Jf$-relative monad that one obtains by considering only the sets $RR(X)$ where $X$ is one of the standard finite sets $stn(n)$,  



it is clearly not what one needs for a mathematical theory of type theories where one wants to minimize the amount of structure used to define the concept of an expression and concentrate on the structures that arise when expressions are used to specify type theories. 

The solution to the problem of expressions that we will use goes back to the foundational works of Aczel \cite{Aczel}, Fiore, Plotkin and Turi \cite{FPT}, Hirschowitz and Maggesi \cite{HM2010} as well as more recent work by Ahrens \cite{Ahrens2016} and Ahrens, Matthes and M\"{o}rtberg.  A good review paper on this surprisingly involved subject is yet to be written. 

For now, we will leave untold the story of the long chain of innovations and modifications that led to the form of the theory of expressions that we use in this paper and restrict ourselves to the description of the result. 

Firstly, we need to choose the set from which we will take the names of free variables. We take this set to be the set $\nat$ of natural numbers. When we refer below to a (name of) free variable $x_i$ it is always assumed that $x_i\in\nat$. Next, we will impose a restriction on the {\em sets} of free variables that we consider. As was seen from the explanation of the meaning of sentences of the forms (\ref{2017.02.06.eq1})-(\ref{2017.02.06.eq5}) the free variables of the component expressions of such sentences are always well ordered. We will use this property to restrict our attention to expressions whose free variables  





Given expressions $F_0,\dots,F_{m-1}$ with free variables from $\{y_0,\dots,y_{n-1}\}$ and an expression $E$ with free variables from $\{x_0,\dots,x_{m-1}\}$ we can substitute $F_i$ for $x_i$ obtaining the expression $E[F_0/x_0,\dots,F_{m-1}/x_{m-1}]$ with free variables from $\{y_0,\dots,y_{n-1}\}$. This operation should satisfy 
...
As has been suggested by earlier authors, such as \cite{MH1995}\footnote{In \cite{MH1995} the author uses categories with families instead of C-systems as the basic structures on which the inference rules define operations. However, the problems discussed below are common to both approaches.}, the objects of the term category should be the ``well-formed'' or ``valid'' contexts of the type theory modulo an equivalence relation determined by the definitional equalities permitted in this type theory.  Morphisms of the term category should be sequences of ``valid typing judgements'' of the type theory modulo a similarly defined equivalence relation and having a certain structure as a sequence.

While these suggestions allow one to define objects and morphisms the construction of the operations and the verification of the axioms of a category was always ``left to the reader''. Indeed, the whole of the very important and in many ways foundational paper \cite{MH1995} does not contain the word ``associativity'' mentioned even once. Similarly, while it is clear that in order for the suggested definitions of objects and morphisms to work and to extend to a definition of a category and then to a definition of a category with families or a C-system, the sets of valid judgements of the theory must satisfy some conditions. These conditions must be spelled out and provably  connected to the possibility of building the categorical structure under consideration from these sets. 

In this paper we describe one of the main steps of the first rigorous mathematical construction of the term model. This construction is applicable to a very general class of type theories both the ones where the sets of valid judgements are inductively generated by inference rules and the ones where where these sets arise from inference rules in a more complex fashion. 

Our construction follows the usual principle that the sets of valid judgements should be defined as sub-quotients of the sets of ``raw'' judgements. The main innovation, other than the first careful mathematical proofs of all the required assertions, is the observations that one can take {\em all} raw judgements as the source for the construction and build from them a C-system. The term C-system of a type theory with a given raw syntax is then a sub-quotient of the raw syntax C-system. The raw syntax C-system can be both directly defined in a way that allows for a straightforward rigorous verification of all the axioms and understood from the perspective of the abstract mathematical theory of C-systems as a particular case of a much more general construction of the presheaf extensions of C-systems. 

Therefore, one divides the construction of the term C-system of a type theory into several steps. Firstly, one has to mathematically model ``the raw syntax''. Secondly, one has to define ``the raw syntax C-system''. 
%###
Thirdly, one has to describe the sets of valid judgements of the five Martin-L\"{o}f kinds in terms of objects associated with the raw syntax C-system.  Fourthly, one has to provide a construction of a sub-quotient of a C-system starting from four objects of the appropriate kind and spell out the conditions that these four objects have to satisfy in order for the construction to work. Fifthly, one has to translate these conditions, in the case of a raw syntax C-system, the back into the language of four ``sets of judgements''.

As an output one then gets a list of conditions on the sets of judgements of four Martin-L\"{o}f kinds and a mathematically rigorous construction of a C-system provided that these conditions are satisfied. 

The first step, finding a good way to mathematically model raw syntax, is discussed in Appendix \ref{}. We base our approach on the ideas of M. Fiore, A. Hirschowitz, M. Maggesi and B. Ahrens. The result of our construction is a function from restricted two-sorted binding signatures, simple combinatorial objects with the two sorts corresponding to the type and element arguments of the constructors of the type theory, to pairs $(\RR,\LM)$ where $\RR$ is a $Jf$-relative monad (see Definition \ref{}) and $\LM$ is a left module over $\RR$ (see Definition \ref{}). The construction is such that for $n\in\nat$, $RR(n)$ is the set of $\alpha$-equivalence classes of element expressions with free variables from the standard set with $n$ elements $stn(n)$ formed using the constructors listed in the binding signature and $\LM(\wh{n})$ is a similar set of $\alpha$-equivalence classes of type expressions. 

The second step is discussed in Sections \ref{},\ref{} and \ref{}. It leads to the construction of the C-system $C(\RR,\LM)$. One could present the construction in a shorter way. We have chosen the approach that shows how this construction can be seen as a special case of more general, but still quite simple, constructions applicable in other areas of the theory of C-systems. 
...
In this paper we describe a purely algebraic construction that defines a C-system $C(\RR,\LM)$ starting with a pair $(\RR,\LM)$ where $\RR$ is a relative monad on the functor $Jf:F\sr Sets$ (see below) and $\LM$ is a (left) module over this monad.
...
On this path one starts by defining from the inference rules a two-sorted binding signature that describes the raw syntax of type and element constructors of the type theory. Then one defines (see Appendix \ref{}) from this two-sorted binding signature a pair of the form $(\RR,\LM)$ and, applying the construction of this paper, obtains the C-system $C(\RR,\LM)$ of the raw syntax of the theory. 

Such C-systems have not been considered previously probably because from the perspective of logic they are hard to interpret. However they provide a very convenient stepping stone to more complex term C-systems of type theories that are defined as regular sub-quotients of $C(\RR,\LM)$. 
