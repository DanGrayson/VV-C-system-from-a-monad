...
planar rooted trees with labels from $Op\cup X$ on the nodes.\editorfootnote{\label{disjoint-labels}
  If the labels were drawn from the disjoint union $Op \amalg X$ instead of
  from the union $Op\cup X$, then the hypothesis that $V\cap Op=\emptyset$ would not be
  needed, we could dispense with $V$, and we could let $X$ be any set.  }
...
monad.\editorfootnote{
  If labels were constructed as suggested in Footnote \ref{disjoint-labels}, then
  we may regard $Exp$ as a function $U \sr U$ and make it into a monad,
  rather than a $J_V$-relative monad.
  }
...
in \cite{AMM2016}. There remains the
...
Let $\C$ and $\D$ be categories. Let $\RR$ be an endo-monad on $\C\times\D$. Let $A\in Ob(D)$. To construct an endo-monad $\RR_{1,A}$ on $\C$.
\end{problem}
%
\begin{construction}
...
$\RR=(RR_{Ob},\eta,rr)$ be a monad on $\C$. A (left) $\RR$-module with values in
...
$\RR=(RR_{Ob},\eta,rr)$ be a $J$-monad. A left module over $\RR$ with values in a
...
module with values in $\E$ over it.  (EDITOR: the previous sentence is incomplete---figure out what it should have said.) 
\end{problem}
%
